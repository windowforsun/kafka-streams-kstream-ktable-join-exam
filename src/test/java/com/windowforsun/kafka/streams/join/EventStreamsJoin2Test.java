...
@ActiveProfiles("test")
public class EventStreamsJoin2Test {
	...
	public void setUp() {
		...
		EventStreamsJoin2 eventStreamsJoin2 = new EventStreamsJoin2(10000L,
			0L,
			"click-topic",
			"view-topic",
			"result-topic");
		eventStreamsJoin2.process(this.streamsBuilder);
	...
	public void viewStream_clickStream_join() {
		...
		assertThat(recordList, hasSize(5));
		...
		assertThat(recordList.get(1).timestamp(), is(7000L));
		assertThat(recordList.get(1).key(), is("F"));
		assertThat(recordList.get(1).value(), is("VIEW:F1, CLICK:F1"));
